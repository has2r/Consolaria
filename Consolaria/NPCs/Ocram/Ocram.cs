		...
		public override void AI()
		{
			...
			else
			{
				...
				else
				{
					...
					{
						npc.damage = (int)(npc.defDamage * 1.1);
						if (npc.ai[1] == 0f)
						{
							float num375 = 14f;
							float num376 = 0.1f;
							...
							if (Main.netMode != 1)
							{
								...
								if (Main.netMode != 1)
								{
									...
									if (npc.localAI[1] > 4f)
									{
										...
										sX += (float)Main.rand.Next(-80, 81) * 0.035f;
										sY += (float)Main.rand.Next(-80, 81) * 0.035f;
										int Type = 83;
										int damage = 35;

										Projectile.NewProjectile(npc.Center.X, npc.Center.Y, sX, sY, Type, damage, 1, Main.myPlayer, 0, 0);
									}
									return;
								}
							}
						...
						else
						{
							if (npc.ai[1] == 1f)
							{
								...
								for (int i = 0; i < 8; ++i)
								{
									Vector2 velocity = Vector2.Normalize(Main.player[npc.target].Center - npc.Center) * 6;
									Vector2 vector32 = new Vector2(npc.position.X + (float)npc.width * 0.5f, npc.position.Y + (float)npc.height * 0.5f);

									Projectile.NewProjectile(npc.Center.X - Main.rand.Next(200), npc.Center.Y - Main.rand.Next(200), velocity.X, velocity.Y, 44, (int)(npc.damage / 2), 0f, Main.myPlayer, 0f, npc.whoAmI);
								}
								Main.PlaySound(15, (int)npc.position.X, (int)npc.position.Y, 0);
								npc.rotation = num319;
								float num384 = 18f;
							...
							for (int playerIndex = 0; playerIndex < 255; playerIndex++)
							{
								if (Main.player[playerIndex].active)
								{
									for (int i = 0; i < MissileProjectiles; i++)
									{
										...
										{
											Num32 = Projectile.NewProjectile(Spawn.X, Spawn.Y, Velocity.X, Velocity.Y, mod.ProjectileType("OcramSkull"), 30, 1f, Main.myPlayer, 0f, 0f); ;
										}
										else
										{
											Num32 = Projectile.NewProjectile(Spawn.X, Spawn.Y, Velocity.X, Velocity.Y, mod.ProjectileType("OcramSkull"), 30, 1f, Main.myPlayer, 0f, 0f); ;
										}
										int Num33 = Projectile.NewProjectile(Spawn.X, Spawn.Y, Velocity.X, Velocity.Y, mod.ProjectileType("OcramSkull"), 30, 1f, Main.myPlayer, 0f, 0f); ;
