...
namespace Consolaria.NPCs
{
	public class ArchWyvernHead : ModNPC
	{
		...
		{
			if (npc.target < 0 || npc.target == 250 || Main.player[npc.target].dead)
			{
			...
			if (Main.netMode != NetmodeID.MultiplayerClient)
			{
				if (npc.ai[0] == 0f)
				{
					...
					{
						int num98 = mod.GetNPC("ArchWyvernBody1").npc.type;
						if (num97 == 4 || num97 == 16)
						{
							num98 = mod.GetNPC("ArchWyvernLegs").npc.type;
						}
						else
						{
							if (num97 == 17)
							{
								num98 = mod.GetNPC("ArchWyvernBody2").npc.type;
							}
							else
							{
								if (num97 == 18)
								{
									num98 = mod.GetNPC("ArchWyvernBody3").npc.type;
								}
								else
								{
									if (num97 == 19)
									{
										num98 = mod.GetNPC("ArchWyvernTail").npc.type;
									}
								}
							}
						}
